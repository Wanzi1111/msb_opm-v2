export default {
  inserted(el, vDir, vNode) {
    ...
    el.addEventListener('focusout', event => { // 此处会在 el-input 的 @change 后执行
      const e = event || window.event
      content = parseFloat(e.target.value) >= 0 ? parseFloat(e.target.value) : -(parseFloat(e.target.value)) // 如果输入负数则转为正数
      if (!content) {
        content = 0.00
      }
    ...
    el.addEventListener('paste', event => {
      ...
      if (arg_max && clipText > arg_max) {
        e.target.value = arg_max
        clipText = arg_max
      }
      if (arg_min && clipText < arg_min) {
        e.target.value = arg_min
        clipText = arg_min
      }
    })
