...
import { sendRepeat } from '@/api/prize/awards'
import { EXPRESS_STATUS_ENUM } from './schema'
...
export default {
  ...
  data() {
    return {
      ...
      activeName: this.$route.query.tab || 'all',
      ...
      gTable: {
        ...
        actionFunction: tableActions({ view: this.view, ship: this.ship })
      },
      forms: forms,
  ...
  watch: {
    activeName(val = 'all', oldVal) {
      this.$router.replace(`/activity-center/awards?tab=${val}`)
      if (val && val !== oldVal) {
        this.listQuery = {
          page: 1,
          limit: 10
  ...
  methods: {
    ...
    getList() {
      ...
      this.$api.getUserPrize({
        ...
        expressStatus: this.tabMap[this.activeName],
        virtualType: 1
      }).then(res => {
