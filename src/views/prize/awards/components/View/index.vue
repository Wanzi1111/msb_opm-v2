 ...
 * @LastEditors: yelei
 * @LastEditTime: 2020-11-07 19:25:47
 * @Description: 获奖信息
-->
  ...
  methods: {
    ...
    getInfo() {
      this.$api.getOneUserPrize({ id: this.id }).then(res => {
        ...
        // teacher: "陈红娜"
        // utime: "1595558892966"
        const prizeType = PRIZE_TYPE_ENUM.find(prizeType => prizeType.value === data.prizeType)
        const prizeTypeValue = prizeType ? prizeType.label : '-'
        this.info = [
          ...
          { label: '奖品类型：', value: prizeTypeValue },
        ...
        this.info1 = [
          ...
          { label: '下单状态：', value: ['', ...EXPRESS_STATUS_ENUM.map(expressStatus => expressStatus.label)][data.expressStatus] },
          { label: '物流单号：', value: data.orderNo },
          { label: '发货渠道：', value: ['', '中台发货', '渠道商发货'][data.expressType] }
        ]
      })
    }
  }
