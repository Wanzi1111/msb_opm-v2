...
import {
  batchNewStudentTaskApproval,
  getNewStudentTaskApprovalList,
  getNewStudentTaskQuerySize
...
export default {
  ...
  computed: {
    ...
    queryData() {
      ...
      // 处理查询条件
      data.beginTime = data.task_time && data.task_time.length ? new Date(data.task_time[0]).getTime() : ''
      data.endTime = data.task_time && data.task_time.length ? new Date(data.task_time[1]).getTime() : ''
  ...
  methods: {
    ...
    // 获取列表
    async getList() {
      const res = await getNewStudentTaskApprovalList(this.queryData)
    ...
    success() {
      ...
      this.$confirm('请确认，是否审核通过?', '提示', {
        ...
        // 通过/驳回
        await batchNewStudentTaskApproval(data)
        this.getList()
        this.$message({
          message: '操作成功',
          type: 'success'
    ...
    // 点击查询总数量
    async searchTotal() {
      const res = await getNewStudentTaskQuerySize(this.queryData)
