<template>
  <div class="app-container">
    ...
    <el-dialog width="650px" title="回访记录" :visible.sync="dialogVisible">
      ...
      <div v-show="echorecord.recordDetails">
        <p class="historyrecord">
          <span>历史记录</span>
          <span v-if="complaints.operationStatus === 2">投诉类型:{{ complaints.complaintsStringType }}</span>
          <span v-if="complaints.operationStatus === 2 && [1,2,3].indexOf(complaints.complaintsType) !== -1">被投诉人:{{ echorecord.recordDetails[0].aimName || '无' }}</span>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      searchs: searchs,
      total: 0,
      act: act({ exportDocument: this.exportDocument, complaintsNumbers: complaintsNumbers, total: this.total || 0 }),
  ...
  methods: {
    ...
    getNumbers() {
      ...
      getComplaintsNumbers({ complaintUser: query.complaintUser }).then(res => {
        if (res.status === 0) {
          const complaintsNumbers = res.data || {}
          const total = this.total
          this.act = act({ exportDocument: this.exportDocument, complaintsNumbers: complaintsNumbers, total: total })
        }
      })
    },
    // 获取投诉列表
