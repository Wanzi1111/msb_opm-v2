  ...
  <div v-loading="appLoading">
    ...
    >
      <div class="item-bar">
        ...
        <el-select v-model="businessType" @change="getGroupTreeByRootId">
          <el-option v-for="v in intelligentMarketing.pushDinc.listBusinessType || []" :key="v.value" :label="v.label" :value="v.value" />
        </el-select>
        <el-button
          class="add-btn"
          type="text"
          size="mini"
          @click="() => append({id: 0, businessType})"
        >
          新增一级分类
        </el-button>
      </div>
...
export default {
  name: 'DialogMarketingPushClassify',
  components: {},
  data() {
    return {
      businessType: '101',
  ...
  methods: {
    getGroupTreeByRootId() {
      listCategory({ businessType: this.businessType })
        .then(res => {
          this.treeData = transferData(res.payload)
        })
    },
    rename(data) {
      this.$prompt('请输入新名称')
    ...
    append(data) {
      this.$prompt('请输入名称')
        .then(({ value }) => {
          const { id } = data
          addCategory({ parentId: id, businessType: this.businessType, name: value })
            .then(() => {
