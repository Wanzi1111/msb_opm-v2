...
export default [
  ...
  },
  {
    ...
    align: 'center',
    minWidth: 120,
    render: (_, data) => store?.state?.intelligentMarketing?.callDincMap?.supplierList_map?.[data]
  },
  {
    prop: 'jobId',
    label: '第三方任务ID',
  ...
  return {
    ...
    taskStatus: (data) => {
      ...
      return [
        {
          type: 'tag',
          label: store?.state?.intelligentMarketing?.callDincMap?.taskStatusList_map?.[data],
          attr: {
            type: colorMap[+data]
          }
        }
