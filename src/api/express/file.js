...
export function exportBackFillData(params) {
  return request({
    ...
    url: '/logistics/manage/exportExpressData',
    method: 'GET',
    params,
 ...
 * @returns {AxiosPromise}
 */
export function exportTemplateSku(data) {
  return request({
    url: '/logistics/manage/exportTemplateSku',
    method: 'POST',
    data,
    responseType: 'blob' // 导出文件必添加、不然request会报错
  })
 ...
 * @returns {AxiosPromise}
 */
export function importCreateOrderSku({ file, onUploadProgress }) {
  console.log(file, onUploadProgress)
  const data = new FormData()
  data.append('file', file)
  return request({
    url: '/logistics/manage/importCreateOrderSku',
    method: 'POST',
    data,
    headers: {
      'Content-Type': 'multipart/form-data'
