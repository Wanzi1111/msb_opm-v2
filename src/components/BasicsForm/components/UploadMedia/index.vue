<template>
  <div class="form-item-element" v-bind="item.attr">
    ...
    <input :ref="item.prop" style="display: none" type="file" :multiple="true" @change="e => uploadMediaFc(e, item)">
    <template v-if="loadingProp.indexOf(item.prop) === -1 && url">
      <div class="media-item">
        ...
        <video v-else-if="item.mediaRules.media === 'audio'" :key="url" controls name="media">
  ...
  methods: {
    ...
    async uploadVerification(e, mediaRules) {
      ...
      try {
        ...
        } else if (mediaRules.case === '<=' && (mediaRules.width || mediaRules.height)) {
          const sizeDat = await getSize()
          const width = parseInt(mediaRules.width)
          const height = parseInt(mediaRules.height)
          if (mediaRules.width && sizeDat.width > width) {
            this.$message.error(`上传尺寸错误，请选择尺寸宽度小于${mediaRules.width} 的图片`)
            return false
          } else if (mediaRules.height && sizeDat.height > height) {
            this.$message.error(`上传尺寸错误，请选择尺寸高度小于${mediaRules.height} 的图片`)
            return false
          }
