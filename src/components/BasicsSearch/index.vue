  ...
  <div>
    <el-row class="search-container">
      ...
      >
        ...
        />
        <el-input v-if="item.type === 'inputSelect'" v-model="form[item.prop]" class="search-item-element" v-bind="item.attr">
          <el-select slot="prepend" v-model="form[item.inputSelectProp.prop]" class="search-item-element-input-select" v-bind="item.inputSelectProp.attr">
            <el-option v-for="(option, optionIndex) in item.inputSelectProp.options" :key="optionIndex" v-bind="option" />
          </el-select>
        </el-input>
        <!--计数器：inputNumber-->
        <el-input-number
      ...
      >
        <el-button v-if="isDownload" :loading="loading" type="primary" icon="el-icon-download" size="small" @click="download">导出</el-button>
        <el-button
          :loading="loading"
          type="primary"
          icon="el-icon-search"
...
const LAYOUT = {
  ...
  middle: { xl: 8, lg: 8, md: 8, sm: 24, xs: 24 },
...
export default {
  ...
  methods: {
    ...
    // 点击搜索函数
    search() {
      const newForm = this.handleSearchValues()
      this.$emit('submitSearch', newForm)
    },
    // 处理提交时、对于一些特殊表单值的过滤
    handleSearchValues() {
      const newForm = cloneDeep(this.form)
      this.searchs.forEach(v => {
        // inputSelect 应该只保留选中的输入值
        if (v.type === 'inputSelect') {
          newForm[newForm[v.inputSelectProp.prop]] = newForm[v.prop]
          delete newForm[v.prop]
          delete newForm[v.inputSelectProp.prop]
        }
      })
      return newForm
    },
    ...
    reset() {
      ...
      const newForm = this.handleSearchValues()
      this.$emit('submitSearch', newForm, true)
    },
    // 获取栏栅布局函数
    getLayout(dat) {
...
.search-item {
  ...
  .search-item-element {
    flex: 1;
    margin-right: 10px;
      ::v-deep .search-item-element-input-select {
          .el-input--suffix{
              width: 112px;
            }
          }
