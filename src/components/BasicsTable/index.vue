  ...
  <div class="basics-table">
    <div v-if="defaultPlugins && defaultPlugins.length" class="table-action">
      <template v-for="(item, index) in defaultPlugins">
        ...
        <template v-else>
          <el-tooltip
            v-if="renderPlugins('download', item)"
            :key="index"
            effect="dark"
            content="下载"
            placement="top"
          ...
          <el-tooltip
            v-if="renderPlugins('colum-height', item)"
            :key="index"
            effect="dark"
            content="密度"
            placement="top"
...
export default {
  ...
  methods: {
    init() {
      ...
      if (plugins === true) {
        this.defaultPlugins = ['download', 'colum-height', 'reload', 'setting']
    ...
    // 设置table rowKey 默认id 否则取列表数据中第一个
    getRowKey(val) {
      const rowkey = val.id ? val.id : uuidv4()
      return rowkey
    }
  }
}
